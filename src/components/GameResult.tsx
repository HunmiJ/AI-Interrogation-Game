...
export function GameResult(props: GameResultProps) {
  ...
  }, [props.accusedNpcId, props.collectedEvidenceIds, props.discoveredContradictionIds, props.discoveredFactIds, props.interviewedNpcIds, props.questionCount])
  ...
  return (
    <main className="result-screen min-h-screen bg-ink px-5 py-10 text-stone-100 md:py-[4.5rem]">
      <div className="result-glow" /><div className="relative mx-auto max-w-5xl">
        <div className="mx-auto max-w-3xl text-center"><div className={`mx-auto flex h-16 w-16 items-center justify-center rounded-full border ${resolution.correct ? 'border-emerald-500/30 bg-emerald-500/10 text-emerald-400' : 'border-danger/30 bg-danger/10 text-danger'}`}>{resolution.correct ? <CheckCircle2 size={30} /> : <XCircle size={30} />}</div><div className="mt-6 text-[10px] font-semibold uppercase tracking-[0.32em] text-brass/70">CASE {resolution.correct ? 'SOLVED' : 'MISJUDGED'} / {gameCase.caseNumber}</div><h1 className="mt-3 font-display text-5xl tracking-[-0.035em] text-stone-50 md:text-7xl">{resolution.correct ? '指认成立' : '真相与你擦肩而过'}</h1><p className="mx-auto mt-5 max-w-xl text-[15px] leading-7 text-stone-400">{resolution.correct ? `证据链评估完成。指认正确并不等于调查完整。` : `你指认了 ${resolution.accusedName}，但完整证据最终指向 ${resolution.culprit.name}。`}</p></div>
